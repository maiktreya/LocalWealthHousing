# from: https://chatgpt.com/share/66f5487a-a8e0-8007-abc4-3341950327fe
...
fr[, age_group := cut(as.numeric(age), breaks = seq(0, 110, by = 5), right = FALSE)]
...
dt[, age_group := cut(age, breaks = seq(0, 110, by = 5), right = FALSE)]
...
total_missing_weight <- sum(age_distribution[!age_distribution$age_group %in% age_distribution2$age_group]$Freq)
# Existing weights from the raked design
raked_weights <- raked_design$variables[, "FACTORCAL"]

# Proportion of each weight
weight_proportions <- raked_weights / sum(raked_weights)

# Redistribute the missing weight
adjusted_weights <- raked_weights + (total_missing_weight * weight_proportions)
raked_design$variables[, "FACTORCAL"] <- adjusted_weights

# Update the weights in the survey design
